...
import { useRotorPhasesList } from './hooks/use-rotor-list.hook'
...
export function RotorPhases() {
	const { id } = useParams()
	const {data} = useRotorPhasesList(+id!)
	...
	const columns: TableColumn[] = useMemo(() => {
		return [
			...
			},
			{
				...
				title: 'Motor Energy Consumption (Kwh)',
				headerClassName: classes.sortable,
				cellClassName: classes.content,
				getValue(item: any) {
			...
			},
			{
				...
				getValue(item: any){
					async function handleDownload(){
						try{
							const res = await http.get(`rotor/${id}/${item.id}/downloadXml`)
							...
							const fileName = match ? match[1] : `rotor_${item.stazioneDiLavorazione}_${item.id}.xml`;
	...
	return (
		<Page title="Subsystem Flange Phases">
			<SmartTable
				columns={columns}
				items={data}
			...
			/>
			{showVerify && <VerifyXml process='rotor' idProcess={parseInt(id!)} idProcessPhase={parseInt(itemId!)} onClose={handleCloseVerify}/>}	
		</Page>
	)
}