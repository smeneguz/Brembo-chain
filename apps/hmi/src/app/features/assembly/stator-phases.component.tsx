...
import { useStatorPhasesList } from './hooks/use-stator-list.hook'
...
export function StatorPhases() {
	const { id } = useParams()
	const {data} = useStatorPhasesList(+id!)
			...
			},
			{
				...
				getValue(item: any){
					async function handleDownload(){
						try{
							const res = await http.get(`stator/${id}/${item.id}/downloadXml`)
							...
							const fileName = match ? match[1] : `stator_${item.stazioneDiLavorazione}_${item.id}.xml`;
	...
	return (
		<Page title="Stator Phases">
			...
			/>
			{showVerify && <VerifyXml process='stator' idProcess={parseInt(id!)} idProcessPhase={parseInt(itemId!)} onClose={handleCloseVerify}/>}	
		</Page>
	)
}